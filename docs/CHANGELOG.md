...
- **Linux "Open Folder" not working** — the instance folder was opened with `xdg-open` without checking the result, so failures were silent. It now verifies the opener's exit code and reports whether it actually opened.
