...
- **Linux console "Connect" failing with "signal only works in main thread"** — the RCON connection (used by the console tab, the readiness check, and the bridge worker) ran the `mcrcon` library inside a worker thread via `asyncio.to_thread`, but upstream `mcrcon` uses `signal.signal`/`signal.alarm` for read timeouts, which is only allowed in the main thread on POSIX. `mcrcon` is now patched to use per-socket timeouts (thread-safe) instead, so the console connects for real instead of silently falling back to log-file output.
...
- **Linux "Start System" after GUI timeout crashed** — the GUI launched `start.bin` as a child process without root, but `start.bin` requires root on Linux, so the Minecraft server failed to initialize ("Cannot load default config file"). The GUI now elevates via `pkexec` (PolicyKit) when it is not already running as root, so the supervisor starts with the privileges it needs.
...
- **Linux "Open Folder" not working** — the instance folder was opened with `xdg-open` without checking the result, so failures were silent. It now verifies the opener's exit code and, if `xdg-open` is unavailable or fails, falls back to other installed file managers (nautilus, dolphin, thunar, nemo, …) so the folder actually opens. If no file manager can be started, the GUI shows the specific reason alongside the folder path.
