...
| `permissions` | Opt-in-Einschränkung der Plugin-API-Oberfläche: `["store", "network", "plugins", "events"]`. **Fehlt das Feld oder ist es leer, läuft das Plugin unbeschränkt** (rückwärtskompatibel). Einmal deklariert wirkt es als Whitelist: Jeder gesperrte Helfer außerhalb der Liste wird mit sicherem Rückgabewert abgelehnt (`PLUGIN-0020`) — analog zu Hook-`permissions`. Siehe [Permissions](./ch03-04-plugin-api.md#permissions-opt-in). |
