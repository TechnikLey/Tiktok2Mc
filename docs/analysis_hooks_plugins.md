...
1. Permission-Modell für Plugins (Enforcement analog Hook-`permissions`; HIGH)
2. Eigene REST-Routen/WebSocket-Anschluss pro Erweiterung (HIGH)
3. Event-Payload-Schemata + Versionsvertrag im Event-Katalog (HIGH)
4. Netzwerk-Client-Bibliothek (Retry/Circuit-Breaker) als Plugin-API exportieren (MEDIUM)
5. UI-Extension-Points für Hooks; Hook↔Hook-Query (MEDIUM/LOW)
