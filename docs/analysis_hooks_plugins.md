...
| *(aktuell)* | **Dashboard-UI-Erweiterungspunkte**: Manifest-Feld `dashboard_ui`; Plugin liefert `get_dashboard_html()` → `POST /plugins/{name}/dashboard-html`, ausgeliefert unter `GET /plugins/{name}/dashboard`; Web-Dashboard erzeugt dynamisch Sidebar-Tabs mit Iframe (lazy-load, „In neuem Tab öffnen“); Referenzimplementierung death-counter | J.3 Nr. 11 / Ecke C.3 Nr. 5: Plugins bekommen Tabs im Dashboard; I.3-UI-Anteil gelöst |
| *(aktuell)* | **RCON-Hintertür autorisiert + dokumentiert**: Config-Gate `rcon.http_command_api` (**Default `false`**) gated `POST /rcon/command` (`403 MC-0012` bei Deaktivierung); Warn-Doku zum Queue/Throttling-Bypass in ch03-04 (EN+DE) | C.3 Nr. 8: Endpunkt ist dokumentiert und standardmäßig deaktiviert |
| *(aktuell)* | **Plugin-Queries mit Korrelations-IDs**: `POST /plugins/{name}/query` → reservierter Befehl `__query__` in der CommandQueue → `on_query()` im Plugin → Antwort per `query-response`; optionale `"queries"`-Deklaration, `504 PLUGIN-0018`/`502 PLUGIN-0019`, Helper `self.query_plugin()`; Referenz death-counter | C.3 Nr. 3: Request/Response zwischen Extensions; C.3 Nr. 4 weitgehend gelöst |
| *(aktuell)* | **Hook-Event-Zugang**: `api.register_event(pattern, fn)` (Patterns wie Plugins) mit Bridge-Fan-Out im Hintergrund-Executor; `api.publish_event(type, data)` mit neuer Permission `events` und Namespacing-Zwang (`<hook>.*`) gegen Event-Fälschung | B.3 Nr. 2 + Nr. 7: Hooks sind jetzt vollwertige Ereignis-Erweiterungsebene |
| *(aktuell)* | **Prozess-Sandbox-Profile**: Built-in-Profile `light`/`moderate`/`strict` in `core/sandbox.py`; global wählbar via `plugin_sandbox.profile`, pro Plugin überschreibbar via `"sandbox_profile"` im plugin.json; Legacy-Rohwerte bleiben Fallback | E.8-Rest / J.2 Nr. 10-Rest: Sandbox ist ohne Hand-Tuning nutzbar |
>
...
5. **Whitelist schneidet legitime Anwendungsfälle ab:** keine `os`/`subprocess`/Audio-/DB-Libs. Gleichzeitig erlaubt `requests` vollen Netzwerkzugriff aus dem Bridge-Prozess — das Sicherheitsversprechen ist also asymmetrisch (Prozess isolation fehlt komplett). **[TEILWEISE gelöst → J.2 Nr. 10]** Prozess-Sandbox-Profile sind jetzt verfügbar (`light`/`moderate`/`strict`, global + pro Plugin) — aber opt-in und die Import-Whitelist bleibt unverändert.
...
7. **`capabilities` rein informativ:** Nirgends wird geprüft; kombiniert mit standardmäßig deaktivierter Sandbox ist das Isolationsmodell opt-in. **[TEILWEISE gelöst → J.2 Nr. 10]** Sandbox-Profile (`light`/`moderate`/`strict`) machen das Aktivieren ohne Hand-Tuning trivial — Standard bleibt weiterhin aus (bewusste Entscheidung); `capabilities` bleiben Discovery-Tags.
...
**Fazit Plugin-System:** Architektonisch die richtige Ebene für alles Schwergewichtige (Threads, Audio, OAuth, Persistenz). **Der Ereigniseingang funktioniert jetzt** (echte Events + Kommentarfeed), Persistenz ist als namespaced Service vorhanden, mit den Dashboard-Tabs (J.3 Nr. 11) haben Plugins einen eigenen UI-Ort im Web-Dashboard und mit dem Query-Kanal (C.3 Nr. 3) können Erweiterungen serverseitig Daten von Plugins abfragen — verbleibende Grenze ist nur noch die Freiheit eigener URL-Schemata/Payload-Formate (C.3 Nr. 4-Rest).
...
- **Status:** (1) ✅ `f409595`, (2) ✅ `022fe7a` (namespaced Store), (3) ✅ **J.3 Nr. 11 erledigt** (Dashboard-Tab mit State-SSE; serverseitige Custom-Routen bleiben Nice-to-have, das Overlay/Dashboard liest den Store bzw. den gepushten State).
- **Original-Urteil:** „Nur mit strukturellen Änderungen".
- **[AKTUALISIERT] Urteil: „Ja"** — Aggregation + Persistenz sind Plugin-DIY mit Board-Mitteln; das Dashboard/Overlay zeigt die Top-10 live per State-Push.
...
3. ~~J.2 Nr. 10 — Capability-Enforcement~~ **[✅ ERLEDIGT]** (`permissions`-Feld, API-Ebene; Prozess-Sandbox-Profile jetzt ebenfalls verfügbar)
4. ~~J.2 Nr. 8 — Request/Response (`api.request()`-Helper) + E.7 (Webhook-Semantik)~~ **[✅ ERLEDIGT]**
5. ~~J.3 nach Bedarf (Nr. 11–14)~~ → **Nr. 14 [✅ ERLEDIGT]** (generisches Event-Abo), **Nr. 12 [✅ ERLEDIGT]** (Katalog-Version + Delivery-Validierung), **Nr. 13 [✅ ERLEDIGT]** (Notification-Dispatcher), **Nr. 11 [✅ ERLEDIGT]** (Dashboard-UI-Erweiterungspunkte); E.7-Restthema „Prozess-Sandbox-Profile" **[✅ ERLEDIGT]** (Built-in-Profile + per-Plugin-Override)
