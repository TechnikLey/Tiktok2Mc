...
4. **Keine eigenen Endpunkte:** Das REST-Interface pro Plugin ist fix (commands/overlay/stream/state/config + dashboard/data). Eigene Abfrage-Routen („gib mir das Leaderboard") sind nicht möglich. **[TEILWEISE gelöst durch J.3 Nr. 11]** Interaktive UIs haben jetzt eine eigene Dashboard-Seite (Tab im Web-Dashboard), die per State-SSE, Commands und Store-API dieselben Bausteine nutzt — echte frei definierte Routen bleiben offen.
