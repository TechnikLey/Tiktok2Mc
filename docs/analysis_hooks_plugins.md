...
> | *(aktuell)* | **Externe Netzwerk-Infrastruktur für Plugins**: `BasePlugin.http_request()` mit Retry + exponentiellem Backoff, 4xx-no-retry, JSON-Parsing und pro-URL-Circuit-Breaker (5 Fehler → 30 s offen); `BasePlugin.ws_connect()/ws_close()` als verwaltete WebSocket-Client-Threads mit Auto-Reconnect und Shutdown-Cleanup (neue optionale Dependency `websocket-client`, lazy importiert); nicht permission-gesperrt (nicht erzwingbar), dokumentiert | Discord-Bots/Game-Connector/Web-Integrationen teilen sich Retry-/Breaker-/Reconnect-Infrastruktur statt Eigenbau pro Plugin |
> | *(aktuell)* | **Hook-zu-Hook-Queries**: `api.register_query(name, fn)` + `api.query_hook(target, query, args)` — synchroner Request/Response-Kanal in-process mit Handler-Isolation (`HOOK-0011`); Queries werden beim Runtime-Reload mitgeräumt | Direkter Datenaustausch zwischen Hooks ohne EventBus-Umweg; Fire-and-Forget bleibt über den Bus abgedeckt |
> | *(aktuell)* | **UI-Extension-Points für Hooks**: neue Permission `ui`; `api.register_dashboard_widget(title, html)` registriert eine HTML-Karte im Dashboard (neue Routen `POST/GET/DELETE /hooks/{name}/widget`, `GET /hooks/{name}/widget.html` als Iframe-Ziel, `GET /hooks/widgets` als Katalog; Deaktivieren räumt auf); GUI rendert eine „Hook Widgets"-Ansicht mit einer Karte pro Hook | Hooks haben jetzt einen offiziellen UI-Integrationspunkt — vorher komplett ohne Dashboard-Präsenz |
>
...
1. WebSocket-*Server* pro Erweiterung (Client-Seite über `ws_connect()` abgedeckt; für Server-seitige WS ist der generische RPC-Kanal + SSE der praktikable Ersatz) — bewusst zurückgestellt, da dynamisches FastAPI-Routing + Auth einen eigenen Design-Aufwand bedeutet.
