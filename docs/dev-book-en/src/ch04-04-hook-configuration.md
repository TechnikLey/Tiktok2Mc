...
> [!NOTE]
> Whether a hook is loaded is managed via the GUI or `POST /api/v1/hooks/{name}/enable` / `disable` and stored in `data/hook_registry.json`. Hooks start disabled by default.
