...
| `permissions` | **Mandatory** declaration of the gated API families the plugin uses: `["store", "network", "plugins", "events"]`. Undeclared families are denied by default (`PLUGIN-0020`, safe fallback) — same semantics as hook `permissions`. See [Permissions](./ch03-04-plugin-api.md#permissions-opt-in). |
...
| `emitted_events` | List of events this plugin publishes to the EventBus. Each entry: `key` (event id, e.g. `"my-plugin.thing"`), `name`, `desc`, `icon`, optional `name_i18n`/`desc_i18n`. **Payload contract:** `version` (integer, default `1` — bump on breaking payload changes) and `data_schema` — a list of declared fields `{key, type, desc, required}` with `type` one of `string`, `number`, `boolean`, `object`, `array`, `any`. Declared events are **enforced**: publishing the type with missing required keys or mistyped values is rejected with `422 API-0010`. The schema is published via `GET /api/v1/reactions/catalog`. |
