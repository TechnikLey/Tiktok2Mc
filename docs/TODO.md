...
### 2. Config Decoupling — Move MinecraftServerAPI Ports to Plugin Config
The `minecraft_server_api` section in `defaults/config.yaml` still holds `web_server_port`, `api_port`, and `enabled`. The two ports (`29187`, `29188`) are plugin-internal — they belong in `defaults/configServerAPI.yml`. Only the bridge `enabled` toggle should remain in the main config. Requires updating `server.py` to read from the plugin config instead of the global config, and updating the auto-update merge logic so `config.yaml` → `config.default.yaml` diff doesn't lose the migration.

### 3. Declarative Command Handler Registration
...
### 6. `core_hash` Build Cache Optimization
...
*Last updated: 2026-05-31 — 592 Python tests + 226 GUI frontend = 818 total*