"""Shared diagnostic types and runtime diagnostics report generation.

Two concerns live in this module:

1. ``Diagnostic`` / ``Severity`` / ``_make_diag`` — lightweight diagnostic
   records used by the .mca parser and validator.
2. The runtime diagnostics report system — a comprehensive snapshot of the
   application's health, including all component states, recent errors,
   crash history, queue statistics, thread statistics, and async task
   information.  This makes debugging production issues significantly
   easier.
"""
...
# ---------------------------------------------------------------------------
#  Runtime diagnostics report generation
# ---------------------------------------------------------------------------
...
def _get_async_tasks() -> list[dict[str, Any]]:
    """Collect statistics about asyncio tasks in the running loop."""
...
def _get_queue_stats(_crash_manager: Any = None) -> dict[str, Any]:
    """Collect queue statistics from the bridge context if available."""
    stats: dict[str, Any] = {}
    try:
        if "src.python.main" in sys.modules:
            ctx = sys.modules["src.python.main"].ctx
            stats["trigger_queue"] = {
...
def generate_diagnostics_report(
    ...
    Returns a dictionary suitable for JSON serialization.  All sections are
    best-effort: a failing collector never raises.
    """
...
def generate_diagnostics_markdown(
    ...
    lines.append(
        f"PID: {report['application']['pid']}  |  "
        f"Platform: {report['application']['platform']}"
    )
    lines.append("")
    ...
    lines.append(
        f"- Components: {health['total_components']} total, "
        f"{health['running']} running, {health['degraded']} degraded, "
    ...
    if "crash_history" in report:
        ...
        for crash in report["crash_history"][-10:]:
            lines.append(
                f"- [{crash.get('code', '?')}] "
                f"{crash.get('exception', crash.get('detail', ''))}"
            )
        lines.append("")
